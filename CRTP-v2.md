...
Starting out on studvm as tech\studentuser, I immediately began enumerating what I could access in the domain. One of the first things I checked was SMB shares available on the domain controller using `Get-SmbShare`. Among the standard administrative shares (ADMIN,C,*C*, IPC$) and domain shares (NETLOGON, SYSVOL), I noticed an interesting non-default share called "maintenance" that stood out. This looked promising since custom shares often contain sensitive files that administrators might not realize are accessible to domain users. I listed the contents of `\\tech-dc.tech.corp\maintenance` and found a PowerShell script named `Backup-ADSystemStateAndGPO.ps1`. When I examined this script, I discovered it contained hardcoded credentials for a local administrator account called `studentadmin` with the password `P@ssS3cretforuservirtualmachineAdm!nthatitisnotguessable!`. This is a common security mistake where administrators embed credentials in scripts for automation purposes without realizing these files might be readable by regular users.

With the studentadmin credentials in hand, I authenticated to studvm using this local administrator account, giving me elevated privileges on the machine. Now running as a local admin, I started preparing the system for further attacks. First, I disabled Windows Defender's real-time monitoring and other security features using `Set-MpPreference` to prevent my tools from being detected and quarantined. I also bypassed AMSI (Antimalware Scan Interface) using an obfuscated PowerShell one-liner that reflectively modifies the AMSI initialization to always return success, effectively blinding PowerShell's malware detection. With defenses down, I added my original domain account (tech\studentuser) to the local Administrators group using `net localgroup Administrators tech\studentuser /add`, ensuring I'd maintain admin access even if the studentadmin account was discovered and disabled. Finally, I used PsExec with the `-s` flag to spawn a SYSTEM-level PowerShell session, giving me the highest possible privileges on the local machine. From this position, I had complete control over studvm and could begin setting up attacks against other systems in the domain, starting with configuring Resource-Based Constrained Delegation on mgmtsrv by modifying its `msds-allowedtoactonbehalfofotheridentity` attribute to allow studvm$ to impersonate any user when accessing it.
...
<img width="795" height="366" alt="image" src="https://github.com/user-attachments/assets/a5a07b00-f54b-4f45-9a5b-081ca4dd7b69" />
...
<img width="841" height="649" alt="image" src="https://github.com/user-attachments/assets/cda7984a-5c65-4524-9e1c-1f3df8f7990f" />
...
With SYSTEM-level access on studvm, I now had the ability to modify Active Directory objects where studvm’s machine account had write permissions. I discovered that studvm (the machine account for studvm) had `GenericWrite` permissions on the mgmtsrv computer object in Active Directory, which is a common misconfiguration where administrators grant excessive permissions without realizing the security implications. This gave me the ability to configure Resource-Based Constrained Delegation (RBCD) on mgmtsrv, essentially telling the Domain Controller that mgmtsrv trusts studvm to impersonate any user when accessing its services.

To set this up, I first needed to install Active Directory management tools on studvm since it was just a regular workstation without these administrative modules by default. I installed the Remote Server Administration Tools (RSAT) features using `Install-WindowsFeature RSAT-AD-PowerShell` and `Install-WindowsFeature RSAT-ADDS`, which gave me access to PowerShell cmdlets for managing Active Directory objects. After importing the ActiveDirectory module, I retrieved the computer objects for both studvm and mgmtsrv using `Get-ADComputer`, then configured RBCD by modifying mgmtsrv's `msDS-AllowedToActOnBehalfOfOtherIdentity` attribute to include studvm's security descriptor. This attribute controls which accounts are trusted for delegation, and by setting it to allow studvm, I was telling Active Directory that mgmtsrv trusts studvm to act on behalf of other users. The key here is that RBCD is a relatively new delegation feature where the target machine (mgmtsrv) specifies who can delegate to it, rather than the delegating machine (studvm) having delegation configured outright, which is why I needed write permissions on mgmtsrv's AD object to configure it.

With RBCD configured, I could now leverage the Kerberos S4U2Self and S4U2Proxy extensions to impersonate any user in the domain when accessing mgmtsrv. First, I ran Mimikatz on studvm to dump credentials from memory and extract studvm’s NTLM hash (520650c3f42f354dc1e53e814d6284aa), which I needed to authenticate as the machine account. Using this hash, I ran Rubeus with the `s4u` command to perform the full S4U attack chain. I requested a TGT for studvm using its hash, then used S4U2Self to get a service ticket for techadmin (the Domain Admin) to studvm$ on behalf of itself, and finally used S4U2Proxy to convert that ticket into a service ticket for techadmin to access the HTTP service on mgmtsrv. The `/altservice:http` flag told Rubeus to substitute the service name from WSMAN to HTTP, which PowerShell Remoting actually uses under the hood.

With the techadmin service ticket injected into my session using `/ptt` (pass-the-ticket), I was able to establish a PowerShell remoting session to mgmtsrv using `Enter-PSSession -ComputerName mgmtsrv.tech.corp`, and when I ran `whoami`, it confirmed I was successfully operating as tech\techadmin, the Domain Admin account. This completely bypassed the need to know techadmin's actual password, I simply abused the delegation trust I had configured combined with studvm$'s machine account hash to impersonate the most privileged user in the domain and gain full administrative access to mgmtsrv.

<img width="1629" height="343" alt="image" src="https://github.com/user-attachments/assets/b5d97fd2-ab3c-4b65-b681-22d76fcb5e1c" />
...
<img width="732" height="291" alt="image" src="https://github.com/user-attachments/assets/c413f2c5-2e28-45f1-a13b-375a06603f77" />

### TechSRV30

Now that I had Domain Admin access on mgmtsrv through the RBCD attack, I started looking for ways to move laterally to other systems in the domain. The first thing I did was set up my workspace on mgmtsrv by copying all my attack tools (Mimikatz, Rubeus, PowerView, and Certify) from studvm to mgmtsrv's C:\Temp directory using PowerShell's `Copy-Item` cmdlet with the `-ToSession` parameter, which let me transfer files directly through my existing PSSession. I created the C:\Temp directory, disabled Windows Defender and AMSI just like I had done on studvm before, and then ran Mimikatz to dump credentials from memory on mgmtsrv. The Mimikatz output revealed several interesting credentials, including the machine account hash for mgmtsrv$ itself, but more importantly, I found credentials for a domain service account called "techservice" with the NTLM hash f8bc230ee35a2e0acf1632b4091e10cd. Service accounts like this are commonly used to run scheduled tasks, web applications, or other automated processes, and they often have elevated privileges or access to multiple systems because they need to perform their duties across the domain.

With the techservice NTLM hash in hand, I headed back to studvm and used Rubeus to request a TGT for this service account using pass-the-hash authentication, then injected the ticket into my current session with the `/ptt` flag. Looking at the ticket details, techservice had a valid TGT that would work for the next ten hours, and based on the service account naming convention (techservice) and my earlier reconnaissance, I suspected this account might have access to techsrv30, which was another server in the domain I hadn't compromised yet. Using the techservice credentials, I was able to establish a connection to techsrv30, demonstrating how service accounts often become valuable pivoting points because they're configured to access multiple systems for legitimate business purposes, but once compromised, they give attackers an easy path to move laterally without triggering alerts that might come from using stolen admin credentials directly. This is a classic example of how proper credential hygiene and limiting service account privileges is crucial—techservice only needed to do specific tasks, but its access was probably broader than necessary, which I could now exploit to continue my attack chain deeper into the tech.corp domain.

<img width="1120" height="506" alt="image" src="https://github.com/user-attachments/assets/a83e6b7a-0f8e-4123-a5bc-c42d7ff77bfb" />
...
<img width="1133" height="513" alt="image" src="https://github.com/user-attachments/assets/c8b3fc14-89ec-485e-b287-5e977ba22988" />

### AdminSRV86

After establishing access to `techsrv30` through the `techservice` account, I ran Mimikatz on that machine to harvest additional credentials from memory. This revealed the machine account `techsrv30$` along with its NTLM hash (75a667d1b05db757150410f320e0ff55), which opened up another interesting attack vector. I started poking around with PowerView to understand what privileges this machine account might have in the domain, and discovered something BloodHound had completely missed, techsrv30$ was a member of the srvusers group. Further enumeration showed that this group had local administrator rights on adminsrv86.tech.corp, giving me a path forward to yet another machine in the domain. This was a good reminder that automated tools like BloodHound are incredibly useful but don't catch everything, especially indirect privilege relationships through group memberships that aren't stored as explicit ACLs.

With the techsrv30 machine account hash in hand, I leveraged it to authenticate to adminsrv86 using a pass-the-hash technique. First, I purged any existing Kerberos tickets from memory to avoid conflicts, then used Rubeus to request a TGT (Ticket Granting Ticket) for techsrv30 by supplying its NTLM hash with the `asktgt` command. Once the TGT was successfully imported into my session, I verified access by listing the C$ administrative share on adminsrv86, confirming I had full local administrator privileges on that machine. I then established a PowerShell remoting session directly to adminsrv86, landing as the techsrv30 machine account with elevated rights. This lateral movement demonstrated how machine accounts, often overlooked in security assessments, can be powerful pivoting points when their group memberships grant them administrative access across multiple systems in the domain. With control over adminsrv86, I was now positioned to continue pushing toward the domain controller and complete domain compromise.
...
<img width="1235" height="658" alt="image" src="https://github.com/user-attachments/assets/fc88aabc-b213-49f6-b8e0-2333f2b13244" />
...
<img width="1717" height="233" alt="image" src="https://github.com/user-attachments/assets/aadb6169-598f-496f-bf23-2e70276303a1" />
...
<img width="825" height="771" alt="image" src="https://github.com/user-attachments/assets/fb8c62ce-36fe-4e6b-b0a0-96f2b4db4747" />

### Tech-DC

Once I had administrative access to adminsrv86 through the techsrv30$ machine account, I immediately ran Mimikatz to dump credentials from memory, hoping to find something useful that could help me move laterally toward the Domain Controller. The credential dump revealed something very interesting, the `causer` account (NTLM hash: 99864b36ef401ee5f15d64e3d4fcd071) had an active logon session on this machine. At first glance, this might seem like just another domain user account, but I remembered from my earlier BloodHound enumeration that causer had special permissions related to the Certificate Authority. I went back and verified with PowerView's `Get-DomainObjectAcl` command, searching specifically for causer's SID (S-1-5-21-1600556212-896947471-994435180-1109) in the Certificate Templates container, and discovered that causer had Certificate-Enrollment rights on both the FIDO and FIDOUsers templates. This was the breakthrough I needed—these templates were configured in an ESC3 vulnerable state, where one template grants "Certificate Request Agent" capabilities (FIDO) and another allows that agent to request certificates on behalf of other users (FIDOUsers). This meant I could potentially abuse Active Directory Certificate Services to escalate all the way to Domain Admin without ever needing techadmin's actual password.

The ESC3 attack is a two-stage certificate abuse technique that exploits a dangerous misconfiguration in ADCS where enrollment agent certificates can be chained together to impersonate any user in the domain. First, I needed to authenticate as causer to request the enrollment agent certificate, so I moved back to my studvm foothold where I had Rubeus available. I used causer's NTLM hash with Rubeus to perform a pass-the-hash attack and request a TGT, then immediately used Certify to request a certificate from the FIDO template with the command `.\Certify.exe request /ca:tech-dc.tech.corp\tech-tech-dc-CA /template:FIDO`. The Certificate Authority validated causer's enrollment rights and issued the certificate successfully with Request ID 9, giving me a PEM-formatted enrollment agent certificate. I converted this to PFX format using OpenSSL on my local Linux machine with the command `openssl pkcs12 -in [causer.pem](http://_vscodecontentref_/0) -keyex -CSP "Microsoft Enhanced Cryptographic Provider v1.0" -export -out causer.pfx`, then base64-encoded it and transferred it back to studvm. With causer's enrollment agent certificate in hand, I now had the golden key and had the ability to request certificates for ANY user in the domain by leveraging the "on behalf of" functionality. I ran Certify again, this time targeting the FIDOUsers template and specifying techadmin as the target user: `.\Certify.exe request /ca:tech-dc.tech.corp\tech-tech-dc-CA /template:FIDOUsers /onbehalfof:tech\techadmin /enrollcert:causer.pfx /enrollcertpw:""`. The Certificate Authority saw causer's valid enrollment agent signature, authenticated the request, and issued a certificate with techadmin as the subject (Request ID 10). I repeated the conversion process to get techadmin.pfx, and then used Rubeus to perform PKINIT authentication: `.\Rubeus.exe asktgt /user:techadmin /certificate:techadmin.pfx /password:"" /domain:tech.corp /ptt`. The Domain Controller validated techadmin's certificate, issued a TGT with full Domain Admin privileges, and injected it into my session. I immediately established a PSSession to tech-dc.tech.corp, ran `whoami` and confirmed I was operating as `tech\techadmin` on the Domain Controller itself, full domain compromise achieved through certificate abuse, never touching techadmin's actual password or hash, and completely bypassing traditional authentication mechanisms through the power of misconfigured ADCS templates.
...
<img width="1159" height="341" alt="image" src="https://github.com/user-attachments/assets/db19365d-8d3b-4fd0-af1a-325408c57b1e" />
...
With Domain Admin access secured on tech.corp, I shifted my focus to the broader environment and started enumerating trust relationships to see if there any potential attack vectors to the `finance.corp` domain. Running `Get-DomainTrust` from the tech-dc Domain Controller revealed a bidirectional trust with finance.corp, which immediately caught my attention as a potential path for further escalation. The trust attributes showed `FILTER_SIDS`, which meant SID filtering was enabled, this is a security feature that blocks privilege escalation across forest trusts by filtering out high-privilege SIDs like Domain Admins or Enterprise Admins from the trusted domain. I ran additional enumeration commands to understand the finance.corp domain structure, discovering it had a single Domain Controller (finance-dc.finance.corp at 172.16.3.4) with one privileged user account called `finadmin` who was a member of the Domain Admins group in that domain. I searched for any obvious attack vectors like SPNs associated with finance, shared computers between domains, or ACL misconfigurations that might give tech.corp users rights in finance.corp, but came up empty, there were no service accounts with SPNs pointing to finance, no shared infrastructure, and no ACL entries granting tech.corp principals any special permissions in the foreign domain.

Since traditional privilege escalation paths were blocked by SID filtering and there were no obvious misconfigurations to exploit, I turned my attention to the trust relationship itself and decided to extract the inter-realm trust keys using Mimikatz. I ran `mimikatz.exe "privilege::debug" "lsadump::trust /patch" "exit"` on tech-dc, which dumped the cryptographic keys used to establish the trust between tech.corp and finance.corp. The output revealed both inbound and outbound trust keys, including the critical RC4-HMAC-NT hash (4ed7f2d467624ea5e196e99af3357017) used for the trust relationship. This trust key is essentially a shared secret between the two domains that allows users from one domain to authenticate to resources in the other domain by requesting inter-realm Ticket Granting Tickets (TGTs). With this trust key in hand, I could forge my own inter-realm TGT that would be accepted by the finance.corp Domain Controller, effectively allowing me to authenticate across the trust boundary. I used Mimikatz's `kerberos::golden` command to create a forged inter-realm TGT: `mimikatz.exe "kerberos::golden /user:Administrator /domain:tech.corp /sid:S-1-5-21-1600556212-896947471-994435180 /rc4:4ed7f2d467624ea5e196e99af3357017 /service:krbtgt /target:finance.corp /ticket:C:\Temp\Tools\trust_tkt.kirbi"`. This command created a golden ticket for "Administrator@tech.corp" (a fictional user that doesn't need to exist) using the trust key, targeting the finance.corp domain's KDC. The resulting ticket was an inter-realm TGT that finance.corp's Domain Controller would accept as legitimate because it was signed with the correct trust key.

With the forged inter-realm TGT created, I used Rubeus to request a service ticket for the CIFS service on finance-dc: `.\Rubeus.exe asktgs /ticket:C:\Temp\Tools\trust_tkt.kirbi /service:cifs/finance-dc.finance.corp /dc:finance-dc.finance.corp /ptt`. This command took my inter-realm TGT and presented it to finance-dc's KDC, requesting a service ticket for file sharing access. The finance.corp Domain Controller validated the inter-realm TGT (since it was signed with the correct trust key), issued a service ticket, and Rubeus injected it into my session. I immediately tested access by running `cmd /c dir \\finance-dc.finance.corp\TechOperations`, and it worked and now I could now browse the TechOperations share that had been inaccessible before. Inside this share, I discovered a PEM file called finadmin.pem, which contained both a private key and a certificate for the finadmin account. This was a massive discovery because it meant finadmin had been issued a certificate for authentication purposes, likely from an ADCS infrastructure in finance.corp. I copied the PEM file contents, transferred them to my local Linux machine, and converted it to PFX format using OpenSSL: `openssl pkcs12 -export -out [finadmin.pfx](http://_vscodecontentref_/1) -in [finadmin.pem](http://_vscodecontentref_/2) -passout pass:`. After base64-encoding the PFX file and transferring it back to tech-dc, I used Rubeus to perform PKINIT authentication as finadmin: `.\Rubeus.exe asktgt /user:finadmin /domain:finance.corp /certificate:finadmin.pfx /password:"" /ptt`. The finance.corp Domain Controller validated finadmin's certificate, issued a TGT with full Domain Admin privileges, and injected it into my session. I verified access with `cmd /c dir \\finance-dc.finance.corp\C$`, successfully listing the C$ administrative share, then established a PSSession directly to finance-dc. Running `whoami` confirmed I was operating as `finance\finadmin` on the finance-dc Domain Controller. I navigated to finadmin's Desktop and found the final flag file: `finalflag.txt` containing `ea62a61c-a75d-41d2-b7fc-172f36c3f6ca`. This cross-forest attack demonstrated the complete compromise chain and I leveraged trust key extraction to forge inter-realm tickets, bypassed SID filtering by not attempting privilege escalation (just requesting legitimate service tickets), discovered certificate credentials in a shared folder, and used PKINIT authentication to achieve Domain Admin access in the finance.corp forest, ultimately compromising both forests in the CRTP exam environment.
...
`*Checking if finadmin has user certificates*`
...
Using the following article to proceed with the attack.
...
Finally we can now read the `TechOperations` share!
...
<img width="1629" height="343" alt="image" src="https://github.com/user-attachments/assets/3dab1f26-ed8a-40a1-af1f-90fd252eeaee" />
...
<img width="852" height="418" alt="image" src="https://github.com/user-attachments/assets/6abb5083-e198-464e-ac97-dfc4404e46c1" />
