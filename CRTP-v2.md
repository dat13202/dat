The CRTP exam simulates a real-world Active Directory penetration test, starting from a standard domain user on a domain-joined workstation (studvm) within the tech.corp domain and progressing toward full Domain Admin compromise across a five-host enterprise environment.

This report details the end-to-end attack path, beginning with initial reconnaissance and credential discovery, followed by the abuse of Resource-Based Constrained Delegation (RBCD) to compromise the management server (mgmtsrv). Leveraging harvested credentials via Mimikatz, I pivoted laterally to additional systems (techsrv30, adminsrv86) and ultimately exploited Active Directory Certificate Services (ADCS) misconfigurations. By abusing the ESC3 enrollment agent vulnerability, I successfully authenticated as the domain administrator and gained full control over the domain controller.

Each phase of the attack builds upon the previous compromise, illustrating how attackers can chain common Active Directory misconfigurations and privilege escalation techniques to methodically escalate from a low-privileged user to complete domain takeover. The scenario highlights practical red team capabilities, including Kerberos ticket abuse, lateral movement, and certificate-based authentication attacks, within a realistic enterprise Active Directory environment.
<img width="1392" height="757" alt="image" src="https://github.com/user-attachments/assets/bd26a0a2-1fee-4660-b903-72380b903325" />

### STUDVM

Starting from studvm as the low-privileged domain user tech\studentuser, I began enumerating accessible resources within the tech.corp domain. An early focus was identifying exposed SMB shares on the domain controller using Get-SmbShare. In addition to standard administrative shares (ADMIN$, C$, IPC$) and default domain shares (NETLOGON, SYSVOL), I identified a non-default share named maintenance, which immediately stood out. Custom shares often contain operational or administrative artifacts that may be unintentionally accessible to regular domain users.

Browsing the contents of \\tech-dc.tech.corp\maintenance, I discovered a PowerShell script titled Backup-ADSystemStateAndGPO.ps1. Upon reviewing the script, I found hardcoded credentials for a local administrator account, studentadmin, along with its password. This represents a common security anti-pattern, where credentials are embedded in automation scripts without proper access controls, exposing them to unintended users.

Using the recovered studentadmin credentials, I authenticated locally to studvm, obtaining local administrator privileges. With elevated access, I prepared the host for subsequent attack stages. First, I disabled Windows Defender real-time protection and related security controls via Set-MpPreference to prevent tool detection and interference. I then bypassed AMSI (Antimalware Scan Interface) using an obfuscated PowerShell one-liner that modifies the AMSI initialization logic to always return a clean result, effectively neutralizing PowerShell-based malware inspection.

To ensure persistence of administrative access, I added my original domain account (tech\studentuser) to the local Administrators group using net localgroup Administrators tech\studentuser /add. This guaranteed continued privileged access even if the exposed studentadmin account were later identified and remediated. Finally, I leveraged PsExec with the -s flag to spawn a SYSTEM-level PowerShell session, granting full control over the compromised workstation.

At this stage, I had complete dominance over studvm, positioning it as a launch point for further domain-wide attacks. With SYSTEM-level privileges established, I proceeded to configure Resource-Based Constrained Delegation (RBCD) on the management server mgmtsrv by modifying its msds-allowedtoactonbehalfofotheridentity attribute. This allowed the studvm$ computer account to impersonate arbitrary users when authenticating to mgmtsrv, setting the foundation for lateral movement and privilege escalation within the domain.
...
With SYSTEM-level access on studvm, I identified that the studvm$ machine account had GenericWrite permissions over the mgmtsrv computer object in Active Directory. This misconfiguration allowed me to configure Resource-Based Constrained Delegation (RBCD) by modifying the msDS-AllowedToActOnBehalfOfOtherIdentity attribute on mgmtsrv, effectively trusting studvm$ to impersonate any user when accessing it.

After installing RSAT and importing the Active Directory PowerShell module, I set up RBCD and then dumped the NTLM hash of studvm$ using Mimikatz. With this hash, I used Rubeus to perform the S4U attack chain (S4U2Self → S4U2Proxy) to impersonate the Domain Admin account (techadmin) and obtain a service ticket for the HTTP service on mgmtsrv.

By injecting the ticket using Pass-the-Ticket, I successfully established a PowerShell remoting session to mgmtsrv as tech\techadmin, achieving full administrative access without knowing the Domain Admin’s password.
...
After gaining admin access to adminsrv86, I dumped credentials with Mimikatz and found an active session for the causer account. Based on earlier BloodHound results, I confirmed that causer had Certificate Enrollment rights on the FIDO and FIDOUsers templates, which were vulnerable to ESC3.

Using causer’s NTLM hash, I authenticated and requested an Enrollment Agent certificate from the FIDO template, then abused it to request a certificate on behalf of techadmin via the FIDOUsers template. With the issued techadmin certificate, I performed PKINIT authentication using Rubeus to obtain a Domain Admin TGT.

I injected the ticket and connected to the Domain Controller as tech\techadmin, achieving full domain compromise via ADCS certificate abuse, without ever obtaining the Domain Admin’s password or hash.
```xml
...
After obtaining Domain Admin access in tech.corp, I enumerated domain trusts and identified a bidirectional forest trust with finance.corp with SID filtering enabled, ruling out direct privilege escalation across the trust. Enumeration revealed a single Domain Controller (finance-dc) and a privileged user (finadmin), but no obvious ACL, SPN, or cross-domain misconfigurations.

I extracted the inter-realm trust key using Mimikatz and forged an inter-realm TGT accepted by finance.corp. Using this ticket, I requested a service ticket to finance-dc and gained access to a shared folder, where I discovered a certificate and private key (finadmin.pem) for the finadmin account.

After converting the certificate to PFX format, I authenticated via PKINIT as finance\finadmin and obtained a Domain Admin TGT. I verified full administrative access on finance-dc and retrieved the final flag, completing a full cross-forest compromise by abusing trust keys and ADCS-based authentication without obtaining any passwords or hashes.
`Enumeration`
